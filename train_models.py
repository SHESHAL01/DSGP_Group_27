...
# Extract values directly from threshold results
classes = threshold_results_log["Class"]

youden_lr = threshold_results_log["Youden Index"].values
youden_rf = threshold_results_rf["Youden Index"].values
...
# This makes path relative to where train_models.py is
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(BASE_DIR, "saved_models/youden_gb_xgb.pkl")

youden_other = joblib.load(MODEL_PATH)

youden_gb = np.array(youden_other["Gradient Boosting"])
youden_xgb = np.array(youden_other["XGBoost"])
...
plt.bar(x - 1.5*width, youden_lr, width, label="Logistic Regression")
plt.bar(x - 0.5*width, youden_rf, width, label="Random Forest")
plt.bar(x + 0.5*width, youden_gb, width, label="Gradient Boosting")
plt.bar(x + 1.5*width, youden_xgb, width, label="XGBoost")

plt.xticks(x, classes, rotation=30)
plt.ylabel("Youden Index")
plt.xlabel("Job Role")
plt.title("Youden Index Comparison Across Models")
plt.ylim(0,1.05)
...
plt.title("Confusion Matrix for Tuned Random Forest Model", fontsize=14)
...
n_trees_range = list(range(1, 301, 5))
...
for n_trees in n_trees_range:
    ...
    train_acc_rf.append(accuracy_score(y_train, train_pred))
    test_acc_rf.append(accuracy_score(y_test, test_pred))


plt.figure(figsize=(10,6))

plt.plot(n_trees_range, train_acc_rf, linewidth=2, label="Train Accuracy")
plt.plot(n_trees_range, test_acc_rf, linewidth=2, label="Test Accuracy")
...
for n_trees in n_trees_range:
    ...
    train_loss_rf.append(log_loss(y_train, train_prob))
    test_loss_rf.append(log_loss(y_test, test_prob))


plt.figure(figsize=(10,6))

plt.plot(n_trees_range, train_loss_rf, linewidth=2, label="Train Log Loss")
plt.plot(n_trees_range, test_loss_rf, linewidth=2, label="Test Log Loss")

plt.xlabel("Number of Trees")
plt.ylabel("Log Loss")
plt.title("Random Forest Log Loss Curve")
